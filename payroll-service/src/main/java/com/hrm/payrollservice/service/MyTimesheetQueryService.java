...
public class MyTimesheetQueryService {

    private final TimesheetMonthRepository monthRepo;
    private final TimesheetDailyRepository dailyRepo;

    public MyMonthTimesheetResponse getMyMonth(Long employeeId, String month) {

        TimesheetMonthEntity monthEntity = monthRepo
                .findByEmployeeIdAndMonth(employeeId, month)
                .orElseThrow(() -> new BusinessException(
                        ErrorCode.NOT_FOUND,
                        "Timesheet month not generated for month=" + month
                ));

        var dailies = dailyRepo.findAllByTimesheetMonthIdOrderByWorkDateAsc(monthEntity.getId());
